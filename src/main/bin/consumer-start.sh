...
base=${bin_abs_path}/..
conf=${base}/conf/consume.properties
...
then
    ...
    echo conf : $conf
    cd $base
    $JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $TRACKER_OPTS -classpath .:$CLASSPATH hackerwin7.beijing.java.kafka.consumer.verify.process.ConsumeHandler 1>>$base/logs/consumer.log 2>&1 &
